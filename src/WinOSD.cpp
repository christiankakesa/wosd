...
int WinOSD::m_delay				= 5;


WinOSD::WinOSD():m_hWnd(NULL), m_hInst(NULL)
{
    this->m_position.x  = static_cast<long>((::GetSystemMetrics(SM_CXSCREEN) - WinOSD::m_width) / 2);
    this->m_position.y  = 0; //static_cast<long>((::GetSystemMetrics(SM_CYSCREEN) - WinOSD::m_height) / 2);
}
...
bool WinOSD::PrintText(const std::string text)
{
    WinOSD::m_text = text;
...
void WinOSD::CreateText(HWND hWnd, std::string text)
{
    WinOSD::m_text = text;
    PAINTSTRUCT ps;
    HDC hdc;
